...
// 🔁 Adaptador y exportación
const adapterFlow = createFlow([encuestaFlow, negacionFlow, defaultFlow])
const adapterDB = new Database()
const botConfig = await createBot({
  flow: adapterFlow,
  provider,
  database: adapterDB
})

// ✅ Exporta el manejador de contexto
export const handleCtx = botConfig.handleCtx

// 🟢 Solo lanza servidor si se ejecuta directamente
if (process.argv[1].includes('app.js')) {
  const PORT = process.env.PORT ?? 3008
  botConfig.httpServer(+PORT)
